...
/**
 *  Class.........: PrivUsuarioPermiso
 *  Database alias: dbdefault
 *  Table.........: priv_usuarios_permisos
 *  Table alias...: pup
 *  Primary key...: id_usuario
 */
class PrivUsuarioPermiso extends AbstractModel
{
    public $id_usuario;
    public $id_permiso;
    ...
    public static function getQuery(array $params = [])
    {
        ...
        $query = Query::selectFrom(['pup' => 'priv_usuarios_permisos']);

        if (isset($params['id_usuario'])) {
            $query->where('pup.id_usuario', $params['id_usuario']);
        }
    ...
    public static function insert(PrivUsuarioPermiso $data, $columns = null)
    {
        $db = DatabaseManager::connect('dbdefault');
        $query = Query::insertInto('priv_usuarios_permisos', $data, $columns);
        $result = $db->executeInsert($query);
        $data->id_usuario = $result->getInsertedId();
        return $result;
    }

    public static function update(PrivUsuarioPermiso $data, $matching = null, $columns = null)
    {
        $db = DatabaseManager::connect('dbdefault');
        $query = Query::update('priv_usuarios_permisos', $data, $matching, $columns);
